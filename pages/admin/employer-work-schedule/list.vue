<template>
  ...
  <div class="flex justify-end">
    <div class="w-15">
      <el-select v-model="type" placeholder="All employer" style="width: 240px">
        <el-option
    ...
    <div class="ml-4">
      <el-button @click="isShowAddSchedule = true">
        <span class="font-normal">+ New Work Schedule</span>
      </el-button>
    </div>
  </div>
  <div class="mt-10 border-2 border-[#DADCDE] rounded-lg">
    <el-table :data="listWorkSchedule" stripe style="width: 100%">
      <el-table-column prop="userName" label="Route Name" />
...
watch(
  ...
  (newVal) => {
    ...
    params.value = { ...params.value, userId: type };
  },
);
...
const listWorkSchedule = ref();
const getListFlight = async () => {
  listWorkSchedule.value = await workScheduleStore.getList(params.value);
  ...
  //   },
  // ];
  // listWorkSchedule.value.forEach((el: any) => {
  //   optionTypeFlight.value = [
  //     ...optionTypeFlight.value,
  //     { value: el.userId, label: el.userName },
  //   ];
  // });
};
await getListFlight();
...
const listStaff = async () => {
  const { data, error } = await useBaseFetch('/users/list-staff', {});
  if (error.value?.data) {
    throw error.value?.data;
  } else {
    const response = data.value as IResponse;
    const listStaff = response.data as object[];
    return listStaff;
  }
};
const listDataStaff = await listStaff();
listDataStaff.forEach((el: any) => {
  optionTypeFlight.value = [
    ...optionTypeFlight.value,
    { value: el.id, label: `${el.firstName} ${el.lastName}` },
  ];
});

const checkExistRoute = (id: number) => {
  optionTypeFlight.value.forEach((el: any) => {
    if (+el.value === id) {
      return true;
    }
  });
